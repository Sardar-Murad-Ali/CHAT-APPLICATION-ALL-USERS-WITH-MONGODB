...
const AppProvider = ({ children }) => {
  ...
  const CreateMessage=async ({message})=>{
      try {
         await authFetch.post("/message",{message})
      } catch (error) {
        dispatch({type:CREATE_MESSAGE_Error,payload:{data:error.res.data.msg}})
        // console.log(error.response.data.msg)
      }
      clearAlert()
    }
    ...
    const allMessages=async ()=>{
      try {
        ...
        dispatch({type:GET_ALL_MESSAGES,payload:{data:data.messages}})
            } catch (error) {
      console.log(error.response.data.msg)      
    }
  }
